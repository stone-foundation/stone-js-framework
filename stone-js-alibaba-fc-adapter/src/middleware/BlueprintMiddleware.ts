 ...
 * @example
 * ```typescript
 * SetAlibabaFcResponseResolverMiddleware(context, next)
 * ```
 */
export const SetAlibabaFcResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === ALIBABA_FC_PLATFORM) {
