...
/**
 * Blueprint middleware that installs the kernel response resolver for the AzureFunctionsHttp platform.
 *
 ...
 */
export const SetAzureFunctionsHttpResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === AZURE_FUNCTIONS_HTTP_PLATFORM) {
...
/**
 * Adapter blueprint middleware for the AzureFunctionsHttp platform.
 */
export const metaAdapterBlueprintMiddleware: Array<MetaMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>> = [
  { module: SetAzureFunctionsHttpResponseResolverMiddleware, priority: 6 }
]
