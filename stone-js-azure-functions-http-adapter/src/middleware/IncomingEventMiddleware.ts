import { IP_HEADERS, AZURE_FUNCTIONS_HTTP_PLATFORM } from '../constants'
...
import { AzureFunctionsHttpAdapterError } from '../errors/AzureFunctionsHttpAdapterError'
import { AzureFunctionsHttpAdapterContext, AzureFunctionsHttpAdapterResponseBuilder } from '../declarations'
...
export class IncomingEventMiddleware {
   ...
   * @throws {AzureFunctionsHttpAdapterError} When the context is missing required components.
   */
  async handle (context: AzureFunctionsHttpAdapterContext, next: NextMiddleware<AzureFunctionsHttpAdapterContext, AzureFunctionsHttpAdapterResponseBuilder>): Promise<AzureFunctionsHttpAdapterResponseBuilder> {
    if (context.rawEvent === undefined || context.incomingEventBuilder?.add === undefined) {
      throw new AzureFunctionsHttpAdapterError('The context is missing required components.')
    }
    ...
    context
      ...
      .add('source', { rawEvent: request, platform: AZURE_FUNCTIONS_HTTP_PLATFORM, rawContext: context.executionContext })
...
/**
 * Meta middleware for processing incoming AzureFunctionsHttp events.
 */
