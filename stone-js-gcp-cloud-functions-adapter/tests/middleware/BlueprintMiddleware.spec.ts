import {
  ...
  SetGcpCloudFunctionsResponseResolverMiddleware
} from '../../src/middleware/BlueprintMiddleware'
import { OutgoingResponse } from '@stone-js/core'
import { GCP_CLOUD_FUNCTIONS_PLATFORM } from '../../src/constants'

describe('SetGcpCloudFunctionsResponseResolverMiddleware', () => {
  ...
  beforeEach(() => {
    ...
    mockBlueprint = {
      get: vi.fn().mockReturnValue(GCP_CLOUD_FUNCTIONS_PLATFORM),
      set: vi.fn()
    }
  })
  ...
  it('should set responseResolver when platform is aws-lambda', async () => {
    ...
    const result = await SetGcpCloudFunctionsResponseResolverMiddleware(context as any, next)
...
describe('metaAdapterBlueprintMiddleware', () => {
  it('should export correct middleware list', () => {
    expect(metaAdapterBlueprintMiddleware).toEqual([
      { module: SetGcpCloudFunctionsResponseResolverMiddleware, priority: 6 }
    ])
  })
})
