 ...
 * @example
 * ```typescript
 * SetGcpCloudFunctionsResponseResolverMiddleware(context, next)
 * ```
 */
export const SetGcpCloudFunctionsResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === GCP_CLOUD_FUNCTIONS_PLATFORM) {
