import { ViewProviderOptions } from '@stone-js/use-view'
import { REACT_VIEW_PROVIDER_KEY } from './constants'
 ...
 *
 * @param options - Composition order and props passed to the provider element.
 ...
 * ```
 */
export const ViewProvider = <T extends ClassType = ClassType>(options: ViewProviderOptions = {}): ClassDecorator => {
  return classDecoratorLegacyWrapper((_target: T, context: ClassDecoratorContext<T>): undefined => {
    setMetadata(context, REACT_VIEW_PROVIDER_KEY, { ...options, isClass: true })
  })
}
