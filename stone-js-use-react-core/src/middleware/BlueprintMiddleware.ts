...
import { defineViewProvider, ViewProviderOptions } from '@stone-js/use-view'
...
import { REACT_PAGE_KEY, STONE_REACT_APP_KEY, REACT_ERROR_PAGE_KEY, REACT_PAGE_LAYOUT_KEY, REACT_VIEW_PROVIDER_KEY } from '../decorators/constants'
/**
 * The build-phase middleware that reads what your classes declared.
 *
 * Turning `@Page` into route definitions, `@PageLayout` into layouts, `@ErrorPage` into
 * error pages and `@ViewProvider` into providers is the same work whatever renders them,
 * so it lives here and both renderers install it. The one middleware that stayed behind in
 * `@stone-js/use-react` is the one wiring the web response hooks.
 */
 ...
 * ```
 */
export const SetReactViewProvidersMiddleware = (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promiseable<IBlueprint> => {
  context
    .modules
    .filter(module => hasMetadata(module, REACT_VIEW_PROVIDER_KEY))
    .forEach(module => {
      const options = getMetadata<ClassType, ViewProviderOptions>(module, REACT_VIEW_PROVIDER_KEY, {})
      context.blueprint.add('stone.useReact.providers', [defineViewProvider(module, { ...options, isClass: true })])
    })
