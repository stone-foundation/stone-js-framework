import {
  ...
  SetGcpCloudFunctionsHttpDefaultAdapterMiddleware,
  SetGcpCloudFunctionsHttpResponseResolverMiddleware
} from '../../src/middleware/BlueprintMiddleware'
import { File } from '@stone-js/filesystem'
import { GCP_CLOUD_FUNCTIONS_HTTP_PLATFORM } from '../../src/constants'
import { OutgoingHttpResponse, BinaryFileResponse } from '@stone-js/http-core'

describe('SetGcpCloudFunctionsHttpDefaultAdapterMiddleware', () => {
  ...
  it('should set default adapter if none exists', async () => {
    ...
    const result = await SetGcpCloudFunctionsHttpDefaultAdapterMiddleware(context as any, next)
  ...
  it('should not overwrite existing default adapter', async () => {
    const adapters = [
      { platform: GCP_CLOUD_FUNCTIONS_HTTP_PLATFORM, default: true },
      { platform: 'lambda' }
    ]
...
describe('SetGcpCloudFunctionsHttpResponseResolverMiddleware', () => {
  let mockBlueprint: any
  let next: any
  ...
  beforeEach(() => {
    ...
    mockBlueprint = {
      get: vi.fn().mockReturnValue(GCP_CLOUD_FUNCTIONS_HTTP_PLATFORM),
      set: vi.fn()
    }
  })
  ...
  it('should set responseResolver when platform is node-http', async () => {
    ...
    const result = await SetGcpCloudFunctionsHttpResponseResolverMiddleware(context as any, next)
...
describe('metaAdapterBlueprintMiddleware', () => {
  it('should export correct middleware list', () => {
    expect(metaAdapterBlueprintMiddleware).toEqual([
      { module: SetGcpCloudFunctionsHttpDefaultAdapterMiddleware, priority: 0 },
      { module: SetGcpCloudFunctionsHttpResponseResolverMiddleware, priority: 6 }
    ])
  })
})
