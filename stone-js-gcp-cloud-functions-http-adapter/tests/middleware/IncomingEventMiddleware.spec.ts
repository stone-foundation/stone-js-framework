...
import { GcpCloudFunctionsHttpAdapterContext, GcpCloudFunctionsHttpAdapterResponseBuilder } from '../../src/declarations'
import {
  getProtocol,
  CookieCollection,
  getHostname,
...
describe('IncomingEventMiddleware', () => {
  ...
  let next: NextMiddleware<GcpCloudFunctionsHttpAdapterContext, GcpCloudFunctionsHttpAdapterResponseBuilder>
  ...
  it('adds the framework-parsed body, the raw payload, and a safe method override', async () => {
    vi.mocked(getHostname).mockReturnValue('localhost')
    vi.mocked(getProtocol).mockReturnValue('http')
    vi.mocked(proxyAddr).mockReturnValue('127.0.0.1')
    vi.mocked(proxyAddr.all).mockReturnValue(['127.0.0.1'])
    vi.mocked(isIpTrusted).mockReturnValue(vi.fn().mockReturnValue(true))
    vi.mocked(CookieCollection.create).mockReturnValue({} as any)

    mockContext.rawEvent.method = 'POST'
    ;(mockContext.rawEvent as any).body = { $method$: 'DELETE', name: 'x' }
    ;(mockContext.rawEvent as any).rawBody = Buffer.from('{"name":"x"}')

    await middleware.handle(mockContext, next)

    expect(mockContext.incomingEventBuilder?.addIf).toHaveBeenCalledWith('body', { $method$: 'DELETE', name: 'x' })
    expect(mockContext.incomingEventBuilder?.add).toHaveBeenCalledWith('metadata', { rawBody: '{"name":"x"}' })
