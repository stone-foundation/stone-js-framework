...
describe('FilesEventMiddleware', () => {
  ...
  it('rebuilds a readable multipart source from rawBody when the request stream is drained', async () => {
    const buffer = Buffer.from('--boundary\r\n...multipart bytes...\r\n--boundary--')
    const fromSpy = vi.spyOn(Readable, 'from')
    vi.mocked(isMultipart).mockReturnValue(true)
    vi.mocked(getFilesUploads).mockResolvedValue({ files: {}, fields: {} } as any)
    ;(mockContext.rawEvent as any).rawBody = buffer

    await middleware.handle(mockContext, next)

    // The drained stream is rebuilt from the captured raw payload, carrying the original headers.
    expect(fromSpy).toHaveBeenCalledWith(buffer)
    const source = fromSpy.mock.results[0].value as any
    expect(source.headers).toBe(mockContext.rawEvent.headers)
    expect(next).toHaveBeenCalledWith(mockContext)
    fromSpy.mockRestore()
  })
})
