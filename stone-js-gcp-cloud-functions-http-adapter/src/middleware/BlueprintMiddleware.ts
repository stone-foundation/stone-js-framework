 ...
 * @example
 * ```typescript
 * SetGcpCloudFunctionsHttpResponseResolverMiddleware(context, next)
 * ```
 */
export const SetGcpCloudFunctionsHttpResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === GCP_CLOUD_FUNCTIONS_HTTP_PLATFORM) {
 ...
 * @example
 * ```typescript
 * SetGcpCloudFunctionsHttpDefaultAdapterMiddleware(context, next)
 * ```
 */
export const SetGcpCloudFunctionsHttpDefaultAdapterMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  ...
  if (!hasDefaultAdapter) {
    context.blueprint.get<AdapterConfig[]>('stone.adapters', []).forEach((adapter) => {
      if (adapter.platform === GCP_CLOUD_FUNCTIONS_HTTP_PLATFORM) {
