import {
  ...
  SetTencentScfResponseResolverMiddleware
} from '../../src/middleware/BlueprintMiddleware'
import { OutgoingResponse } from '@stone-js/core'
import { TENCENT_SCF_PLATFORM } from '../../src/constants'

describe('SetTencentScfResponseResolverMiddleware', () => {
  ...
  beforeEach(() => {
    ...
    mockBlueprint = {
      get: vi.fn().mockReturnValue(TENCENT_SCF_PLATFORM),
      set: vi.fn()
    }
  })
  ...
  it('should set responseResolver when platform is aws-lambda', async () => {
    ...
    const result = await SetTencentScfResponseResolverMiddleware(context as any, next)
