 ...
 * @example
 * ```typescript
 * SetAzureFunctionsResponseResolverMiddleware(context, next)
 * ```
 */
export const SetAzureFunctionsResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === AZURE_FUNCTIONS_PLATFORM) {
