...
import { AzureFunctionsAdapterError } from '../../src/errors/AzureFunctionsAdapterError'
import { IncomingEventMiddleware } from '../../src/middleware/IncomingEventMiddleware'
import { AzureFunctionsAdapterContext, AzureFunctionsAdapterResponseBuilder } from '../../src/declarations'

describe('IncomingEventMiddleware', () => {
  let middleware: IncomingEventMiddleware
  let mockContext: AzureFunctionsAdapterContext
  let next: NextMiddleware<AzureFunctionsAdapterContext, AzureFunctionsAdapterResponseBuilder>
  ...
  beforeEach(() => {
    ...
    } as unknown as AzureFunctionsAdapterContext
  ...
  it('should throw error if context is missing rawEvent or incomingEventBuilder', async () => {
    ...
    await expect(middleware.handle(mockContext, next)).rejects.toThrow(AzureFunctionsAdapterError)
