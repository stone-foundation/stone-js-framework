...
describe('RawResponseWrapper', () => {
  it('builds a Response with status, headers and body', async () => {
  ...
  it('defaults to 500 and a null body, and appends multiple Set-Cookie', () => {
...
describe('ServerResponseMiddleware', () => {
  ...
  it('throws when the context is incomplete', async () => {
    const mw = new ServerResponseMiddleware()
    await expect(mw.handle({} as any, (async () => ({})) as any)).rejects.toThrow(FetchAdapterError)
  })
})
