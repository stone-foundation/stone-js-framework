...
import { FetchErrorHandler } from '../src/FetchErrorHandler'
import { FetchAdapterError } from '../src/errors/FetchAdapterError'
import { IncomingEventMiddleware } from '../src/middleware/IncomingEventMiddleware'
import { SetFetchResponseResolverMiddleware } from '../src/middleware/BlueprintMiddleware'
...
describe('IncomingEventMiddleware', () => {
  ...
  it('builds the incoming event from a Web Request', async () => {
    ...
    expect(data.source.platform).toBe(FETCH_PLATFORM)
  ...
  it('throws when the context is incomplete', async () => {
    const mw = new IncomingEventMiddleware({ blueprint: blueprintStub() })
    await expect(mw.handle({} as any, (async () => builderStub()) as any)).rejects.toThrow(FetchAdapterError)
  })
})

describe('SetFetchResponseResolverMiddleware', () => {
  it('installs the response resolver for the fetch platform', async () => {
    const blueprint = blueprintStub({ 'stone.adapter.platform': FETCH_PLATFORM })
    const next = vi.fn(async () => blueprint)
    await SetFetchResponseResolverMiddleware({ blueprint } as any, next as any)
  ...
  it('does nothing for another platform', async () => {
    ...
    await SetFetchResponseResolverMiddleware({ blueprint } as any, (async () => blueprint) as any)
    expect(blueprint.set).not.toHaveBeenCalled()
  })
})

describe('FetchErrorHandler', () => {
  const handler = (): FetchErrorHandler => new FetchErrorHandler({
