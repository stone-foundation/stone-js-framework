 ...
 */
export const SetAlibabaFcHttpResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === ALIBABA_FC_HTTP_PLATFORM) {
...
/**
 * Adapter blueprint middleware for the AlibabaFcHttp platform.
 */
export const metaAdapterBlueprintMiddleware: Array<MetaMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>> = [
  { module: SetAlibabaFcHttpResponseResolverMiddleware, priority: 6 }
]
