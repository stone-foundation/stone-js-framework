...
describe('the connection a Redis store opens', () => {
  afterEach(async () => { await RedisCacheStore.disconnect() })

  it('is opened once for one target, however many stores point at it', async () => {
    // The store is rebuilt with the container, on every event. Without pooling, a busy server opens a
    // TCP connection per request and keeps opening them. A connection is a resource, not state: the
    // values are in Redis, so reusing one loses nothing and saves a handshake.
    vi.resetModules()
    let built = 0
    class FakeRedis {
      constructor () { built++ }
    ...
    const { RedisCacheStore: Fresh } = await import('../../src/drivers/RedisCacheStore')

    await Fresh.create({ name: 'a', driver: 'redis', url: 'redis://one:6379' } as any).get('k')
    await Fresh.create({ name: 'b', driver: 'redis', url: 'redis://one:6379' } as any).get('k')

    expect(built).toBe(1)

    await Fresh.create({ name: 'c', driver: 'redis', url: 'redis://two:6379' } as any).get('k')
  ...
  it('leaves a client the application built to the application', async () => {
    // Its lifetime is not this store's business: whoever opened it closes it, and nothing here does.
    h.client.get.mockResolvedValue(JSON.stringify({ v: 'kept' }))

    await RedisCacheStore.disconnect()

    await expect(store().get('k')).resolves.toEqual({ v: 'kept' })
    expect(h.client.quit).toBeUndefined()
  })
})
