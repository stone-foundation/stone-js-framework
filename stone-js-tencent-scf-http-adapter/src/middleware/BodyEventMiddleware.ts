...
export class BodyEventMiddleware {
   ...
   */
  private toNodeMessage (rawEvent: TencentScfHttpEvent): IncomingMessage {
   ...
   * @throws {TencentScfHttpAdapterError} If the body parsing fails or is invalid.
   */
  private getBody (message: IncomingMessage, rawEvent: TencentScfHttpEvent): unknown {
    ...
    if (rawBuffer.byteLength > limit) {
      throw new TencentScfHttpAdapterError('Body payload exceeds configured limit.')
    }
   ...
   */
  private getRawBuffer (rawEvent: TencentScfHttpEvent, encoding: BufferEncoding): Buffer {
   ...
   * @throws {TencentScfHttpAdapterError} If parsing fails.
   */
  private parseBodyContent (type: string | false, buffer: Buffer, encoding: BufferEncoding): unknown {
    try {
      ...
      throw new TencentScfHttpAdapterError('Failed to parse request body.', { cause: error })
    }
  }
}
