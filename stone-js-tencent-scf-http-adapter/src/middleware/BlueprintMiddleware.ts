 ...
 * @example
 * ```typescript
 * SetTencentScfHttpResponseResolverMiddleware(context, next)
 * ```
 */
export const SetTencentScfHttpResponseResolverMiddleware = async (
  context: BlueprintContext<IBlueprint, ClassType>,
  next: NextMiddleware<BlueprintContext<IBlueprint, ClassType>, IBlueprint>
): Promise<IBlueprint> => {
  if (context.blueprint.get<string>('stone.adapter.platform') === TENCENT_SCF_HTTP_PLATFORM) {
